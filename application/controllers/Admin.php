...
class Admin extends CI_Controller
{
        ...
        for ($i = 1; $i <= $page; $i++) {
            ...
            if ($i >= 2) {
                ...
                if ($row > 0) {
                    ...
                    $doc = new DOMDocument("1.0", "utf-8");
                    $doc->formatOutput = true;
                    $doc->appendChild($doc->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="https://sic88.org/assets/css/css_sitemap.xsl"'));
                    ...
                    foreach ($sitemap as $key => $val) {
                        ...
                        $name = $doc->createElement("loc");
                        $name->appendChild($doc->createTextNode('https://sic88.org/' . $val['name']));
                        $url->appendChild($name);
                        $lastmod = $doc->createElement("lastmod");
    ...
    public function sitemap_page()
    {
        ...
        $url = $doc->createElement("url");
        $name = $doc->createElement("loc");
        $name->appendChild($doc->createTextNode('https://sic88.org/'));
        $url->appendChild($name);
        $lastmod = $doc->createElement("lastmod");
